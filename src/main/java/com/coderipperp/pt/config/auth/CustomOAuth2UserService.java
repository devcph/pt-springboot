...
import com.coderipperp.pt.config.auth.dto.OAuthAttributes;
import com.coderipperp.pt.config.auth.dto.SessionUser;
import com.coderipperp.pt.domain.user.User;
import com.coderipperp.pt.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        //      - 구글인지 네이버인지 등등을 구분하기 위함
        // 2. userNameAttributeName
        //      - OAuth2 로그인 진행 시 키가 되는 필드값. Primary Key와 같은 의미
        //      - 구글은 기본 코드 "sub"을 지원하지만, 네이버나 카카오는 지원 안 함
        //      - 네이버와 구글 동시 지원할 때 사용
        // 3. OAuthAttributes
        ...
        //      - 직렬화를 구현하여 저장해야 하기 때문에 따로 클래스를 만들어줌
